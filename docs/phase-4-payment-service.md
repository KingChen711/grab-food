...
- `src/database/migrations/<timestamp>-InitialSchema.ts` — write the `up()` (CREATE TABLE …) and `down()` (DROP TABLE …). Easiest path: define the entities (with `@Index()` / `unique: true` per workflow A above) and run `pnpm --filter @grab/payment-service db:generate` to scaffold the migration from them, then review/tidy it. (Or hand-write it in the `InitialSchema` style.) **Add useful indexes** — `payments(order_id)`, `payments(idempotency_key)` unique, `wallet_transactions(wallet_id)`, `payouts(recipient_id, period_start)`. Two valid workflows in this repo (both end up in a migration — never `synchronize`):
  - **(A) Decorator + generate (recommended here):** declare `@Index()` / `@Index({ unique: true })` (or `unique: true` on the column) on the entity, then run `pnpm --filter @grab/payment-service db:generate`. TypeORM diffs the entities against the DB and writes the `CREATE INDEX` into the generated migration. **Review the generated SQL before committing** — generate output can be noisy.
  - **(B) Hand-written SQL:** no decorator; write `CREATE INDEX` after each `CREATE TABLE`, matching the polished `InitialSchema` migrations.
    Note: with `synchronize: false`, an `@Index()` decorator has no effect _at runtime on its own_ — it only takes effect once `db:generate` reads it into a migration. So if you hand-write migrations and never generate, skip the decorator; if you generate, the decorator is the source of truth.
